...
function processing () {
	...
	#mmdebstrap --skip=check/empty --components="main restricted universe multiverse" --variant=minbase --include="console-setup,locales,tzdata,keyboard-configuration" "${SUITE}" "${MOUNT_POINT}" "${MIRROR1}"

	apt-get install -y debootstrap
	debootstrap --variant=minbase --include="console-setup,locales,tzdata,keyboard-configuration" "${SUITE}" "${MOUNT_POINT}" "${MIRROR1}"
